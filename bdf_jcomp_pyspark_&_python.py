# -*- coding: utf-8 -*-
"""BDF_JCOMP_PYSPARK_&_PYTHON.ipynb
...
def plot_sentiment_gauge(sentiment):
    fig, ax = plt.subplots(figsize=(6, 3))
    ax.axis("off")

    # Gauge colors
    ax.barh(0, sentiment + 1, color='green' if sentiment > 0 else 'red', height=0.3)
    ax.barh(0, 1, color='gray', height=0.3, alpha=0.2)

    plt.xlim(-1, 1)
    plt.title(f"🎭 Sentiment Polarity Gauge\n(Sentiment = {sentiment:.2f})", fontsize=14, weight='bold')
    plt.text(sentiment, 0.1, "▲", fontsize=18, ha='center', color='black')
    plt.show()


plot_sentiment_gauge(TextBlob(title).sentiment.polarity)
...
def plot_best_time_heatmap(country_insights, country):
    sub = country_insights[country_insights["country"] == country]
    if sub.empty:
        print("No data available for this country.")
        return

    pivot = sub.pivot_table(values="is_trending", index="day", columns="hour", aggfunc="mean")
    plt.figure(figsize=(10, 5))
    sns.heatmap(pivot, cmap="YlGnBu", annot=True, fmt=".2f")
    plt.title(f"Best Upload Time Heatmap for {country}", fontsize=14, weight='bold')
    plt.ylabel("Day of Week")
    plt.xlabel("Hour of Day")
    plt.show()

plot_best_time_heatmap(country_insights, country)
...
# Replace with your actual recorded times
times = {'pyspark': pyspark, 'python': python}  # in seconds

# Create figure
plt.figure(figsize=(7,5))

# Plot bars
bars = plt.bar(times.keys(), times.values(), color=['orange', 'skyblue'], edgecolor='black', width=0.5)

# Add chart title and labels
plt.title('Execution Time Comparison: PySpark vs Python', fontsize=14, weight='bold')
